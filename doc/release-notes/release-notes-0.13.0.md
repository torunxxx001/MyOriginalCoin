MyOriginalCoin Core version 0.13.0 is now available from:
...
In addition to that, with 0.12.x there have been varied reports of MyOriginalCoin Core
randomly crashing on Windows XP. It is [not clear](https://github.com/torunxxx001/MyOriginalCoin/issues/7681#issuecomment-217439891)
...
myoriginalcoin-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/myoriginalcoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The MyOriginalCoin Core code base has
...
HD wallets are incompatible with older versions of MyOriginalCoin Core.

[Pull request](https://github.com/torunxxx001/MyOriginalCoin/pull/8035/files), [BIP 32](https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki)
...
proposed parameters, a future release of MyOriginalCoin Core is expected that
...
The two phases are distinct in the MyOriginalCoin-Qt GUI. During the first one,
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/torunxxx001/MyOriginalCoin/pull/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for myoriginalcoin-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use MORGC/kB (MarcoFalke)
...
- #7692 `29b2be6` Remove P2P alert system (morgcdrak)
...
- #8275 `042c323` Remove bad chain alert partition check (morgcdrak)
...
- #7619 `18b3f1b` Add missing sudo entry in gitian VM setup (morgcdrak)
- #7616 `639ec58`  [depends] Delete unused patches  (MarcoFalke)
- #7658 `c15eb28` Add curl to Gitian setup instructions (morgcdrak)
...
- #7613 `3798e5d` Add autocomplete to myoriginalcoin-qt's console window (GamerSg)
...
- #8256 `af2421c` BUG: myoriginalcoin-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from myoriginalcoind (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the MyOriginalCoin-Core repository and website to the About Dialog (MarcoFalke)
...
- #7531 `18cb2d5` Add bip68-sequence.py to extended rpc tests (morgcdrak)
...
- #7773 `7c80e72` Fix comments in tests (morgcdrak)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for myoriginalcoind procs (MarcoFalke)
...
- #8164 `0f24eaf` [MyOriginalCoin-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of myoriginalcoind startup (laanwj)
...
- #7540 `ff46dd4` Rename OP_NOP3 to OP_CHECKSEQUENCEVERIFY (morgcdrak)
