MyOriginalCoin Core version 0.17.0 is now available from:

  <https://github.com/torunxxx001/bin/bitcoin-core-0.17.0/>
...
installer (on Windows) or just copy over `/Applications/MyOriginalCoin-Qt` (on Mac)
or `myoriginalcoind`/`myoriginalcoin-qt` (on Linux).
...
be used in V0.17 by starting myoriginalcoind with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed MyOriginalCoin Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for MyOriginalCoin transactions that are not fully signed
...
Overall, the construction of a fully signed MyOriginalCoin transaction goes through the
following steps:
...
- An **Extractor** produces a valid MyOriginalCoin transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in MORGC. This new field deprecates previous fee fields, such as
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart myoriginalcoind with
...
- When running myoriginalcoind with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If myoriginalcoind
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in MORGC (mryandao)
...
- #13528 `b877c39` Move MyOriginalCoinGUI initializers to class, fix initializer order warning (laanwj)
...
- #13604 `75bea05` Add depends 32-bit arm support for myoriginalcoin-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling myoriginalcoin-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use MyOriginalCoinTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use MyOriginalCoinTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use MyOriginalCoinTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use MyOriginalCoinTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for MyOriginalCoinTestFramework (WillAyd)
...
- #12102 `7fb8fb4` Apply hardening measures in myoriginalcoind systemd service file (Flowdalic)
- #12652 `55f490a` myoriginalcoin-cli: Provide a better error message when myoriginalcoind is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to myoriginalcoind in debian package (ghost)
...
- #13603 `453ae5e` myoriginalcoin-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix myoriginalcoin-cli --version (Empact)
...
- #12877 `2b54155` Use myoriginalcoind in Tor documentation (knoxcard)
...
- #13742 `d2186b3` Adjust github.com/torunxxx001 links (MarcoFalke)
...
- #13905 `73a09b4` fixed myoriginalcoin-cli -help output for help2man (hebasto)
