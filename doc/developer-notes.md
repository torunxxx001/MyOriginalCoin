...
- [Development guidelines](#development-guidelines)
    - [General MyOriginalCoin Core](#general-bitcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
MyOriginalCoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/torunxxx001/MyOriginalCoin/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/myoriginalcoind -printtoconsole
```
...
MyOriginalCoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/torunxxx001/MyOriginalCoin/issues/12691)
...
However, in open source software such as MyOriginalCoin Core, where everyone uses
...
pay attention to for reviewers of MyOriginalCoin Core code.

General MyOriginalCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: MyOriginalCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of MyOriginalCoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against MyOriginalCoin Core so that they can be integrated
...
file descriptors it will cause problems with MyOriginalCoin's `select()` loop, because
...
compatibility between nodes. This happened in MyOriginalCoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to MyOriginalCoin's
...
Commit [`bb81e173`](https://github.com/torunxxx001/MyOriginalCoin/commit/bb81e173) is an example of a scripted-diff.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `myoriginalcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `myoriginalcoin-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of MyOriginalCoin Core, the wallet was always
