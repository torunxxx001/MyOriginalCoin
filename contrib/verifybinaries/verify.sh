#!/usr/bin/env bash
# Copyright (c) 2016 The MyOriginalCoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   github.com/torunxxx001 and bitcoin.org and compares them.
...
RCSUBDIR="test"
HOST1="https://github.com/torunxxx001"
...
if [ $? -ne 0 ]; then
   echo "bitcoin.org failed to provide signature file, but github.com/torunxxx001 did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitcoin.org and github.com/torunxxx001 signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the MyOriginalCoin Core binary release signing key installed?"
   fi
